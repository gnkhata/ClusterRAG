...
def create_preprocessor(tokenizer, max_length, model_name):
    def preprocess_dataset(examples):
        ...
        if "t5" in model_name:
            ...
            texts = [src + "\n" + tgt for src, tgt in zip(inputs, targets)]
            tokenized = tokenizer(
                texts,
            ...
            model_inputs = {
                ...
                "labels": labels["input_ids"],  # causal LM predicts next token
            }
        return model_inputs
...
class GeneralSeq2SeqDataset(Dataset):
    ...
    def __init__(self, data_addr, use_profile, task, mode, create_prompt = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        if self.use_profile:
            return {
                ...
                "source" : self.create_prompt(self.data[index]['input'], self.data[index]['profile'], self.task, self.mode),
